...
export default defineContentScript({
    ...
    main(ctx: ContentScriptContext) {
        ...
        async function retrieveAndDispatch(reAttempt: boolean = false) {
            ...
            const { currentSubtitleData } = (await browser.storage.local.get('currentSubtitleData')) as {
                currentSubtitleData?: { url?: string; data?: VideoData; timestamp?: number };
            };
        ...
        async function waitForFetchCompletion(maxWaitMs: number = 20000): Promise<boolean> {
            ...
            while (Date.now() - start < maxWaitMs) {
                const { currentSubtitleFetchStatus } = (await browser.storage.local.get(
                    'currentSubtitleFetchStatus'
                )) as { currentSubtitleFetchStatus?: { inProgress?: boolean } };
